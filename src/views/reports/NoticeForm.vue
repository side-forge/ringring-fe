...
const loadNoticeData = async () => {
  if (isEditMode.value) {
    try {
      // TODO: API 호출하여 기존 데이터 가져오기
      // const response = await api.getNotice(noticeId.value)
      // formData.value = response.data

      // 임시 더미 데이터 (실제로는 API에서 가져올 데이터)
      const dummyData = {
        title: `공지사항 제목 ${noticeId.value}`,
      ...
      formData.value = { ...dummyData }
      console.log(`공지사항 ${noticeId.value}번 데이터 로드됨`)
    } catch (error) {
...
const handleSubmit = async () => {
  ...
  try {
    if (isEditMode.value) {
      // 수정 모드
      console.log('공지사항 수정:', formData.value)
      alert('공지사항이 수정되었습니다.')

      // TODO: API 호출 로직 추가
      // await api.updateNotice(noticeId.value, formData.value)
    } else {
      // 등록 모드
      ...
      // TODO: API 호출 로직 추가
      // await api.createNotice(formData.value)
    }
        ...
        <div class="form-content">
          ...
          <div class="form-row">
            ...
            <input
                ...
                type="text"
                class="form-input"
                placeholder="이미지나 영상을 보여줄 링크를 입력하세요."
            />
          </div>
